...
def get_presets(dir="color_list"):
    files = glob.glob(os.path.join(dir, '*.plist'))
    presets = {}
    for f in files:
...
class ClutDialog(wx.Frame):
    def __init__(self, parent, id, histogram, init, end, wl, ww):
        super(ClutDialog, self).__init__(parent, id, "CLUT")
    ...
    def _init_gui(self):
        self.cb_presets = wx.ComboBox(self, -1)

        self.clut_widget = CLUTImageDataWidget(self, -1, self.histogram,
                                               self.init, self.end, self.wl, self.ww)

        sizer = wx.BoxSizer(wx.VERTICAL)
        sizer.Add(self.cb_presets, 0, wx.EXPAND)
    ...
    def __bind_events_wx(self):
        self.cb_presets.Bind(wx.EVT_COMBOBOX, self.OnSelectPreset)

    def _populate_presets(self):
        self.presets = get_presets()
        self.cb_presets.SetItems(sorted(self.presets.keys()))

    def OnSelectPreset(self, evt):
        bcb = evt.GetEventObject()
        idx = evt.GetInt()
        st  = bcb.GetString(idx)

        preset = self.presets[st]

        self.clut_widget.SetPlistProject(preset)
